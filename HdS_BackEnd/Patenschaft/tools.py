...
import csv, json
from Patenschaft.schemas import Patenschaftsantrag, Geschlecht, Abbuchung, Zahlungsweise

def CheckIfAntragIsComplete(antrag: Patenschaftsantrag):
    isAntragFilled = True

    if len(antrag.kind) == 0:
...
def writeAntragCSVfromJSON(antrag: Patenschaftsantrag):
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"Patenschaftsantrag_{timestamp}.csv"
    
    with open(f".\Antrag\{filename}", mode="w", newline="", encoding="utf-8") as file:
        file.write(f"Kind: {antrag.kind}\n")
...
def writeAntragStringfromJSON(antrag: Patenschaftsantrag):
    stringAntrag =  f"Kind: {antrag.kind}\n" + \
                    f"Vorname: {antrag.vorname}\n" + \
